...
from configs import solver_config as _sc_tier2
...
# ── Process-pool worker (hardware-fastened build) ──────────────────────────
# Module-level (picklable) worker for the ProcessPoolExecutor path. The thread
# path uses a local closure; processes need a top-level function and picklable
# args. All inputs (Route, CarState, spline/KDTree providers, loop-geometry
# DataFrames, and the simple kwargs) pickle, so the result is identical to the
# thread path — only the parallelism model changes.
def _mp_sample_day(payload):
    args, car, kwargs = payload
    d = args["d"]
    result = sample_day(
        args["route"], car, args["solar_provider"], args["wind_provider"],
        d, args["plan_to_use"], args["s0_d"], args["alpha"],
        is_today=args["is_today"], dist_done_km=args["dist_done_km"],
        elapsed_s=args["elapsed_s"], cs_taken=args["cs_taken"],
        loop_geoms=args["loop_geoms"], **kwargs)
    return d, result
...
def sample_all_days(routes: list, car: CarState, solar_providers: dict, wind_providers: dict,
    ...
    def _prep_args(d: int) -> dict:
        """Build the picklable sample_day argument bundle for day d. Runs in the
        parent process (thread path calls it inline; process path pickles the
        returned dict). Kept side-effect-free so both paths behave identically."""
        ...
        return dict(
            route=route, solar_provider=solar_providers.get(d),
            wind_provider=wind_providers.get(d), d=d, plan_to_use=plan_to_use,
            s0_d=s0_d, alpha=alpha, is_today=is_today, dist_done_km=d_dist,
            elapsed_s=d_elap, cs_taken=d_cs, loop_geoms=(loop_geoms_by_day or {}).get(d))

    def _prepare_and_sample(d: int) -> tuple[int, dict]:
        a = _prep_args(d)
        t0 = time.perf_counter()
        result = sample_day(
            a["route"], car, a["solar_provider"], a["wind_provider"], d,
            a["plan_to_use"], a["s0_d"], a["alpha"], is_today=a["is_today"],
            dist_done_km=a["dist_done_km"], elapsed_s=a["elapsed_s"],
            cs_taken=a["cs_taken"], loop_geoms=a["loop_geoms"], **kwargs)
        dt = time.perf_counter() - t0
    ...
    _use_processes = bool(getattr(_sc_tier2, "TIER2_USE_PROCESS_POOL", False))
    ...
    if n_days <= 1 or n_workers <= 1:
        # Single day or single core — skip pool overhead.
        for d in tqdm(day_indices, desc="Tier2 days"):
            _, result = _prepare_and_sample(d)
            per_day[d] = result
    elif _use_processes:
        # TRUE multi-core (hardware-fastened): fan the days out over PROCESSES,
        # sidestepping the GIL that throttles the thread path. Payloads are
        # built in the parent (picklable dicts) and the module-level
        # _mp_sample_day worker runs sample_day in each process. Identical
        # results; wall-clock scales with min(n_days, cores).
        #
        # SAFETY FALLBACK: if the process pool fails for ANY reason — most
        # likely a pickling error under the Windows/macOS 'spawn' start method
        # if some provider isn't picklable — we log it and transparently fall
        # back to the thread path so the run ALWAYS completes with identical
        # results (just without the multi-core speedup on that machine).
        logger.info("Tier2: sampling %d days with %d PROCESSES (fastened)", n_days, n_workers)
        try:
            payloads = {d: (_prep_args(d), car, kwargs) for d in day_indices}
            with ProcessPoolExecutor(max_workers=n_workers) as pool:
                futures = {pool.submit(_mp_sample_day, payloads[d]): d for d in day_indices}
                with tqdm(total=n_days, desc="Tier2 days") as pbar:
                    for fut in as_completed(futures):
                        d, result = fut.result()
                        per_day[d] = result
            ...
            logger.warning(
                "Tier2 process pool failed (%s: %s) — falling back to the thread "
                "path for this run. Results are unaffected; you just won't get "
                "the multi-core speedup here. (If on Windows, this is the 'spawn' "
                "pickling limitation.)", type(_mp_err).__name__, _mp_err)
            per_day.clear()
            _run_with_threads()
    else:
        _run_with_threads()
