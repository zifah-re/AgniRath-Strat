...
# Speed at which the trailer/tow truck moves the car through trailered stretches
# (km/h). No energy is drawn/stored while trailered, but this time IS counted
# into the ETA. Sourced from solver_config so it's a tunable knob, not a magic
# number (strategist-set: 80 km/h). getattr fallback keeps old callers working.
_TRAILER_SPEED_KMH = getattr(_sc_forward, "TRAILER_TOW_SPEED_KMH", 80.0)
...
class DayEvalResult:
    ...
    # on, so downstream code can partition the day into stage1 / loop / stage2
    # segments for per-stage reporting. Empty when no route (synthetic days).
    seg_type_trace: np.ndarray = dataclasses.field(
        default_factory=lambda: np.array([]))
...
def simulate_variable_speed(v_kmh: np.ndarray, route: Route, car: CarState,
    ...
    # reported total_breakdown_s just reads 0 for that deterministic run).
    _skip_breakdown = (
        rng is None
        and not getattr(_sc_forward, "INCLUDE_BREAKDOWN_IN_TIME", False)
        and getattr(_sc_forward, "SKIP_BREAKDOWN_WHEN_UNUSED", False))
    ...
    for seg_i, v in enumerate(v_kmh):
        ...
        for k in range(n_substeps):
            ...
            # BreakdownModel call — skip for trailered segments (motor off) and
    ...
    # Per-point seg_type in ONE vectorized lookup on the simulated route (the
    # loop-spliced route, if loops were committed), so per-stage reporting can
    # split the day into stage1 / loop / stage2 without touching the hot loop.
    if use_route and len(x_array):
        try:
            _seg_type_trace = route.seg_type_array(np.asarray(x_array, dtype=float)).astype(str)
        except Exception:
            _seg_type_trace = np.array([])
    else:
        _seg_type_trace = np.array([])
