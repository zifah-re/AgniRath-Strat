...
def check_gradient_feasible(route: RouteProfile, max_safe_grade_pct: float = 8.0) -> bool:
    """A 60 km/h constant-speed plan needs no segment steep enough that the
    car can't hold speed on the climb, nor so steep downhill that regen
    braking alone can't hold 60 (ASSUMPTION: +/-8% is our safety cutoff for
    ...
    print(f"  {route.name}: gradients OK (max {route.gradient_pct.max():.2f}%, "
          f"min {route.gradient_pct.min():.2f}%), 60 km/h profile is safe.")
    return True


def drive_leg_energy_wh(route: RouteProfile, start_utc: datetime, solar_series,
                         n_laps: int = 1) -> dict:
    """Drive `route` at TARGET_SPEED_MS, n_laps times back-to-back (for the
    ...
    f_aero = 0.5 * AIR_DENSITY * CDA_M2 * TARGET_SPEED_MS ** 2
    f_grav = MASS_KG * G * np.sin(theta)
    f_total = f_rr + f_aero + f_grav                      # N, +ve = motoring load

    p_wheel = f_total * TARGET_SPEED_MS                    # W, per segment while traversing it
    seg_time_s = seg_m / TARGET_SPEED_MS
...
def run_day(n_loops: int, start_pack_voltage_v: float, verbose: bool = True) -> dict:
    ...
    if verbose:
        print("=" * 78)
        print("ROUTE GRADIENT SAFETY CHECK (60 km/h constant-speed plan)")
        print("=" * 78)
    ...
    leg1 = drive_leg_energy_wh(stage1, depart_utc, solar_s1, n_laps=1)
    record(f"Drive Stage 1 ({leg1['distance_km']:.1f} km @ 60 km/h)", leg1["net_wh"],
    ...
    if n_loops <= 0:
        ...
        for i in range(n_loops):
            leg_lap = drive_leg_energy_wh(loop, loop_start, solar_loop, n_laps=1)
            record(f"Loop {i + 1}/{n_loops} ({leg_lap['distance_km']:.1f} km @ 60 km/h)",
                   leg_lap["net_wh"],
    ...
    leg2 = drive_leg_energy_wh(stage2, stage2_start, solar_s2, n_laps=1)
    record(f"Drive Stage 2 ({leg2['distance_km']:.1f} km @ 60 km/h)", leg2["net_wh"],
...
def trace_floor_crossing(route: RouteProfile, start_utc: datetime, solar_series,
                          soc_start_pct: float) -> dict | None:
    ...
    p_wheel = f_total * TARGET_SPEED_MS
    seg_time_s = seg_m / TARGET_SPEED_MS
