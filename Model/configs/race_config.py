...
# Soft finish limit (strategist directive 23/08): "nothing beyond 17:10 unless
# a dire scenario; 17:30 is the absolute hard cutoff." Arriving 17:00-17:10 is
# tolerated only when the extra distance outweighs the penalty (priced in
# Tier 3); the planner must not push a normal day past 17:10. Day 8 has NO soft
# band — its timed finish is a hard 15:00 (no penalties are acceptable on the
# final day), so the soft limit collapses onto 15:00 there.
SOFT_FINISH_BAND_MIN = 10                # minutes past 17:00 still allowed (days 1-7)

def day_finish_soft_limit_s(day_index: int) -> int:
    """Latest finish a NORMAL plan may target — 17:10 on days 1-7, 15:00 on
    Day 8. Sits between the on-time target (day_finish_time_s) and the absolute
    parc-ferme cutoff (day_finish_cutoff_s); the finish-feasibility backstop
    drops loops to keep the true finish (drive + mandatory stops) within it."""
    if day_index == N_RACE_DAYS - 1:
        return DAY8_TIMED_FINISH_S                      # Day 8: hard 15:00, no band
    return FINISH_TIME_S + SOFT_FINISH_BAND_MIN * 60    # 17:10
