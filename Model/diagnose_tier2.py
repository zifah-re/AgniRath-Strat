...
# # ========== TEST 5: Day 7 specifically ==========
# print("\n" + "=" * 70)
# print("TEST 5: singleday.solve — Day 7, 0 loops, start=48%, alpha=20%")
# print("=" * 70)

# route7_files = sorted(glob.glob(os.path.join(save_dir, "*Day 7*.save")), key=_route_sort_key)
# weather7_files = glob.glob(os.path.join(json_dir, "*Day 7*.json"))

# if route7_files:
#     route7 = _load_route(route7_files, 7)
#     solar7 = HourlyJSONSolarProvider(weather7_files, route7)
#     wind7 = HourlyJSONWindProvider(weather7_files, route7)
#     print(f"Route7: {route7.df.shape[0]} pts, {route7.df['distance_m'].max()/1000:.1f} km")

#     try:
#         res = singleday.solve(
#             route=route7, car=car, solar_provider=solar7, wind_provider=wind7,
#             day_index=6, start_soc_pct=48.0, alpha_next_day_pct=20.0,
#             loops_committed=[],
#         )
#         for k, v in res.items():
#             if isinstance(v, (int, float, str, bool)):
...
# # Also test with higher start SOC
# print("\n" + "=" * 70)
# print("TEST 6: singleday.solve — Day 7, 0 loops, start=80%, alpha=20%")
# print("=" * 70)
...
#         res = singleday.solve(
#             route=route7, car=car, solar_provider=solar7, wind_provider=wind7,
#             day_index=6, start_soc_pct=80.0, alpha_next_day_pct=20.0,
#             loops_committed=[],
#         )
#         for k, v in res.items():
#             if isinstance(v, (int, float, str, bool)):
...
files = sorted(glob.glob(os.path.join(save_dir, "*Day 7*.save")))
for f in files:
    with open(f, 'r') as fh:
        d = json.load(fh)
    dists = d["profile"]["Distance"]
    print(f"{os.path.basename(f)}: {len(dists)} pts, {dists[0]:.3f} - {dists[-1]:.3f} km")